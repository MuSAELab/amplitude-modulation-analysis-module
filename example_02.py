...
if __name__ == "__main__":
    ...
    # Power using FFT
    X = np.fft.fft(x)
    power_X = float( (1 / n**2) * sum(X * np.conj(X)) )
    
    # Power using its PSD from rFFT
    psd_rfft_r = rfft_psd(x, fs, win_funct = 'boxcar')
    ...
    # Power using its PSD from rFFT
    psd_rfft_b = rfft_psd(x, fs, win_funct = 'blackmanharris')
    f_step = psd_rfft_r['freq_delta']
    ...
    w_size =  1 * fs
    w_shift = 0.5 * w_size
    rfft_spect_h = strfft_spectrogram(x, fs, w_size, w_shift, win_funct = 'hamming' )
    ...
    w_size =  1 * fs
    w_shift = 0.5 * w_size
    rfft_spect_r = strfft_spectrogram(x, fs, w_size, w_shift, win_funct = 'boxcar')
    ...
    w_size =  1 * fs
    w_shift = 0.5 * w_size
    rfft_mod_b = strfft_modulation_spectrogram(x, fs, w_size, w_shift, win_funct_y = 'boxcar', win_funct_x = 'boxcar')
    ...
    wav_mod_6 = wavelet_modulation_spectrogram(x, fs, 6, win_funct_x = 'boxcar')
